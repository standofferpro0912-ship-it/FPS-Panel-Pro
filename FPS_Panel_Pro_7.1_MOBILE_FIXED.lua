...
local Config = {
    ...
    HUDOpacity = 0.96,
    TacticalHUD = true,
    HideTacticalHUDWhenPanelOpen = true,
    MobileHUD = true,
...
local function SetVisible(v)
    visible=v
    if type(Env.FPSPanelApplyHUDVisibility)=="function" then pcall(Env.FPSPanelApplyHUDVisibility) end
    if v then
...
local function UpdateFOVRing()
    local visible=Config.TacticalHUD and Config.FOVRing and Config.Aimbot and CombatHUD.Enabled
    local cam=workspace.CurrentCamera
    local center=Vector2.new((cam and cam.ViewportSize.X or 0)*.5,(cam and cam.ViewportSize.Y or 0)*.5)
    local radius=math.clamp(tonumber(Config.AimFOV) or 180,10,600)
    if cam and Config.MobileHUD and UserInputService.TouchEnabled then
        radius=math.min(radius,math.min(cam.ViewportSize.X,cam.ViewportSize.Y)*.42)
    end
    local n=#FOVSegments
...
local function UpdateRadar()
    local hudOn=Config.TacticalHUD and CombatHUD.Enabled
    RadarRoot.Visible=hudOn and Config.Radar
...
local function UpdateThreat(dt)
    if not Config.TacticalHUD or not CombatHUD.Enabled or not Config.ThreatWarning then Threat.Visible=false;return end
    Threat.Visible=true
...
local function UpdateTargetHUD()
    if not Config.TacticalHUD or not CombatHUD.Enabled or not Config.TargetHUD then TargetPanel.Visible=false;return end
...
local function ScanCombatFeed()
    if not Config.TacticalHUD or not CombatHUD.Enabled or (not Config.DamageFeed and not Config.DeathFeed) then FeedRoot.Visible=false;return end
...
local function UpdateCompass()
    if not Config.TacticalHUD or not CombatHUD.Enabled or not Config.Compass then CompassRoot.Visible=false;return end
...
local function ApplyHUDSafeZone()
    ...
    local compact=Config.MobileHUD and (isTouch or view.X<900 or view.Y<520)
    local inset=Config.HUDSafeZone and (compact and 10 or 8) or 0
    local hudScale=1
    if Config.MobileHUD and isTouch then
        hudScale=math.clamp(short/720,.72,1)
    end
    CombatHUDScale.Scale=hudScale

    local radarSize=math.clamp(tonumber(Config.RadarSize) or 150,100,230)
    if compact then radarSize=math.min(radarSize,112) end
    RadarRoot.Size=UDim2.fromOffset(radarSize,radarSize)
    RadarRoot.Position=UDim2.fromOffset(compact and 10 or 18,inset+(compact and 10 or 18))

    if compact then
        Threat.Size=UDim2.fromOffset(220,44)
        Threat.Position=UDim2.new(.5,-110,inset+10)
        CompassRoot.Size=UDim2.fromOffset(280,24)
        CompassRoot.Position=UDim2.new(.5,-140,inset+61)
        TargetPanel.Size=UDim2.fromOffset(212,66)
        TargetPanel.Position=UDim2.new(.5,-106,1,-78)
        FeedRoot.Size=UDim2.fromOffset(205,92)
        FeedRoot.Position=UDim2.new(1,-215,1,-103)
        Watermark.Size=UDim2.fromOffset(145,20)
        Watermark.Position=UDim2.new(1,-155,inset+8)
        Watermark.TextSize=8
        WatermarkAccent.Position=UDim2.new(1,-158,inset+11)
    else
        Threat.Size=UDim2.fromOffset(286,54)
        Threat.Position=UDim2.new(.5,-143,inset+18)
        CompassRoot.Size=UDim2.fromOffset(360,30)
        CompassRoot.Position=UDim2.new(.5,-180,inset+83)
        TargetPanel.Size=UDim2.fromOffset(246,72)
        TargetPanel.Position=UDim2.new(.5,-123,1,-92)
        FeedRoot.Size=UDim2.fromOffset(230,112)
        FeedRoot.Position=UDim2.new(1,-248,1,-132)
        Watermark.Size=UDim2.fromOffset(180,22)
        Watermark.Position=UDim2.new(1,-195,inset+10)
        Watermark.TextSize=9
        WatermarkAccent.Position=UDim2.new(1,-198,inset+13)
    end
end

local function ApplyTacticalHUDVisibility()
    local enabled=Config.TacticalHUD and not (Config.HideTacticalHUDWhenPanelOpen and visible)
    CombatHUD.Enabled=enabled
    if not enabled then
        RadarRoot.Visible=false
        Threat.Visible=false
        TargetPanel.Visible=false
        FeedRoot.Visible=false
        CompassRoot.Visible=false
        Watermark.Visible=false
        WatermarkAccent.Visible=false
        for _,seg in ipairs(FOVSegments) do seg.Visible=false end
    ...
    UpdateWatermark()
    UpdateFOVRing()
    Env.FPSPanelApplyHUDVisibility=ApplyTacticalHUDVisibility
end
...
Toggle(Pages.Interface,"FOV Ring","Render a clean segmented FOV guide around the cursor.",function() return Config.FOVRing end,function(v) Config.FOVRing=v;UpdateFOVRing();ApplyTacticalHUDVisibility() end)
...
Toggle(Pages.Interface,"Tactical Radar","Top-left proximity map.",function() return Config.Radar end,function(v) Config.Radar=v;UpdateRadar();ApplyTacticalHUDVisibility() end)
...
Toggle(Pages.Interface,"Compass","Show a subtle heading compass.",function() return Config.Compass end,function(v) Config.Compass=v;UpdateCompass();ApplyTacticalHUDVisibility() end)
Toggle(Pages.Interface,"Threat Warning","Show the nearest enemy warning.",function() return Config.ThreatWarning end,function(v) Config.ThreatWarning=v;UpdateThreat(0);ApplyTacticalHUDVisibility() end)
...
Toggle(Pages.Interface,"Target HUD","Show the currently locked target.",function() return Config.TargetHUD end,function(v) Config.TargetHUD=v;UpdateTargetHUD();ApplyTacticalHUDVisibility() end)
Toggle(Pages.Interface,"Damage Feed","Show observed enemy health reductions.",function() return Config.DamageFeed end,function(v) Config.DamageFeed=v;ScanCombatFeed();ApplyTacticalHUDVisibility() end)
Toggle(Pages.Interface,"Death Feed","Show observed player deaths.",function() return Config.DeathFeed end,function(v) Config.DeathFeed=v;ScanCombatFeed();ApplyTacticalHUDVisibility() end)
...
Toggle(Pages.Interface,"Watermark","Show a small panel watermark.",function() return Config.Watermark end,function(v) Config.Watermark=v;UpdateWatermark();ApplyTacticalHUDVisibility() end)
...
Toggle(Pages.Interface,"Mobile Safe Zone","Adds extra top safe-area spacing.",function() return Config.HUDSafeZone end,function(v) Config.HUDSafeZone=v;ApplyHUDSafeZone();ApplyTacticalHUDVisibility() end)
Toggle(Pages.Interface,"Mobile Tactical HUD","Master switch for radar, threat, target HUD, feed, compass and watermark.",function() return Config.TacticalHUD end,function(v) Config.TacticalHUD=v;ApplyTacticalHUDVisibility() end)
Toggle(Pages.Interface,"Hide HUD When Menu Open","Hide tactical widgets while the panel is open.",function() return Config.HideTacticalHUDWhenPanelOpen end,function(v) Config.HideTacticalHUDWhenPanelOpen=v;ApplyTacticalHUDVisibility() end)
...
local enhancedLayoutT=0
local enhancedFovT=0
local function GetEnhancedESPInterval()
    local base=math.max(.03,tonumber(Config.ESPUpdateRate) or .06)
    if Config.MobileHUD and UserInputService.TouchEnabled then base=math.max(base,.10) end
...
RunService:BindToRenderStep("FPSPanel_EnhancementHUD",Enum.RenderPriority.Last.Value,function(dt)
    ...
    if enhancedLayoutT>=.35 then
        enhancedLayoutT=0
        ApplyHUDSafeZone()
        ApplyTacticalHUDVisibility()
    end
    if enhancedFovT>=.10 then
        enhancedFovT=0
        UpdateFOVRing()
    end

    local radarInterval=.10
    if Config.AdaptiveRadar then
        local fps=tonumber(fpsValue) or 60
        if fps<30 then radarInterval=.25 elseif fps<45 then radarInterval=.16 end
        if UserInputService.TouchEnabled then radarInterval=math.max(radarInterval,.14) end
    end
...
local EnhancementDefaults={
    ...
    WatermarkText="FPS PANEL PRO",AdaptiveESP=true,AdaptiveRadar=true,HUDSafeZone=true,HUDOpacity=.96,TacticalHUD=true,HideTacticalHUDWhenPanelOpen=true,MobileHUD=true,
}
...
-- Final HUD sync: settings always win over stale overlay state.
pcall(function() ApplyTacticalHUDVisibility() end)
