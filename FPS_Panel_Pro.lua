...
Toggle(Pages.Visuals,"ESP","Master switch for the local ESP system.",function() return Config.ESP end,function(v)
    Config.ESP=v
    -- UpdateESP owns the hard-reset path. Defer it so this callback also works
    -- during the early UI construction phase before the ESP system is created.
...
Toggle(Pages.Visuals,"Legacy Boxes","Compatibility option; off by default.",function() return Config.ESPBoxes end,function(v) Config.ESPBoxes=v; task.defer(function() pcall(UpdateESP) end) end)
Toggle(Pages.Visuals,"Legacy Tracers","Compatibility option; off by default.",function() return Config.ESPTracers end,function(v) Config.ESPTracers=v; task.defer(function() pcall(UpdateESP) end) end)
...
--========================================================
-- ESP uses its own unscaled ScreenGui because WorldToViewportPoint returns
-- real viewport pixels. Keeping it outside the panel UIScale prevents all
-- screen-space ESP elements from being shifted/scaled on mobile.
...
local function RefreshESPPlayerCache()
    ...
    for _,plr in ipairs(Players:GetPlayers()) do
        if plr~=LocalPlayer then table.insert(espPlayerCache,plr) end
    end
end
RefreshESPPlayerCache()
Players.PlayerAdded:Connect(function(plr) if plr~=LocalPlayer then table.insert(espPlayerCache,plr) end end)
Players.PlayerRemoving:Connect(function(plr)
    for i=#espPlayerCache,1,-1 do if espPlayerCache[i]==plr then table.remove(espPlayerCache,i) end end
end)
...
local function RemoveESP(player)
    ...
    -- IMPORTANT: several ESP primitives live directly under espOverlay,
    -- not under e.container. Destroy every primitive explicitly or stale
    -- boxes/tracers will survive character respawns / ESP recreation.
    ...
    for _,line in ipairs(e.legacyBox or {}) do
        if line then line:Destroy() end
    end
    for _,line in ipairs(e.skeleton or {}) do
        if line then line:Destroy() end
    end
...
local function CreateESP(player)
    ...
    local legacyBox={}
    for i=1,8 do
        legacyBox[i]=New("Frame",{BackgroundColor3=GetESPColor(player),BorderSizePixel=0,Visible=false,ZIndex=60},espOverlay)
    end
    local skeleton={}
...
local function GetCharacterScreenBounds(cam,char,view)
    -- Use the model's real 3D bounding box instead of estimating size from
    -- HumanoidRootPart depth. The old estimate produced floating corner
    -- brackets that were visibly detached from the character.
    ...
    local hx=boundsSize.X*.5
    local hy=boundsSize.Y*.5
    local hz=boundsSize.Z*.5
    ...
    for ix=-1,1,2 do
        for iy=-1,1,2 do
            for iz=-1,1,2 do
                local worldPoint=boundsCF*CFrame.new(ix*hx,iy*hy,iz*hz).Position
    ...
    minX=math.clamp(minX,-50,view.X+50)
    maxX=math.clamp(maxX,-50,view.X+50)
    minY=math.clamp(minY,-50,view.Y+50)
    maxY=math.clamp(maxY,-50,view.Y+50)
...
local function UpdateLegacyBox(e,minX,minY,maxX,maxY,color)
    ...
    local defs={
        {x=minX,y=minY,w=cw,h=2},
        {x=minX,y=minY,w=2,h=ch},
        {x=maxX-cw,y=minY,w=cw,h=2},
        {x=maxX-2,y=minY,w=2,h=ch},
        {x=minX,y=maxY-2,w=cw,h=2},
        {x=minX,y=maxY-ch,w=2,h=ch},
        {x=maxX-cw,y=maxY-2,w=cw,h=2},
        {x=maxX-2,y=maxY-ch,w=2,h=ch},
    }

    for i,d in ipairs(defs) do
        local v=seg[i]
        v.AnchorPoint=Vector2.new(0,0)
        v.Position=UDim2.fromOffset(d.x,d.y)
        v.Size=UDim2.fromOffset(d.w,d.h)
...
local function MakeSkeletonLine(parent)
    return New("Frame",{
        AnchorPoint=Vector2.new(.5,.5),
        BackgroundColor3=C.Accent,
        BorderSizePixel=0,
        Size=UDim2.fromOffset(1.5,1),
        Visible=false,
        ZIndex=55,
    },parent)
end
...
local function GetSkeletonBones(char)
    local isR15=char and char:FindFirstChild("UpperTorso") and char:FindFirstChild("LowerTorso")
    return isR15 and R15Bones or R6Bones
end

local function UpdateSkeleton(e,char,color,cam)
    if not e.skeleton then return end
    if not Config.ESPSkeleton or not char or not cam then
    ...
    for i,pair in ipairs(defs) do
        ...
        if a and b then
            if not line then
                line=MakeSkeletonLine(espOverlay)
                e.skeleton[i]=line
            end
            ...
            if za>0 and zb>0 then
                ...
                local inA=va.X>=-100 and va.X<=viewport.X+100 and va.Y>=-100 and va.Y<=viewport.Y+100
                local inB=vb.X>=-100 and vb.X<=viewport.X+100 and vb.Y>=-100 and vb.Y<=viewport.Y+100
                if not (inA and inB) then
                    line.Visible=false
                    continue
                end
                local d=vb-va
                local len=d.Magnitude
                if len>0.5 then
                    line.Position=UDim2.fromOffset((va.X+vb.X)*.5,(va.Y+vb.Y)*.5)
                    line.Size=UDim2.fromOffset(math.clamp(len,1,300),1.25)
    ...
    for i=#defs+1,#e.skeleton do
        e.skeleton[i].Visible=false
    end
end

UpdateESP=function()
    -- HARD RESET FIRST: every render pass starts by hiding all optional ESP
    -- primitives.
    for _,e in pairs(espObjects) do
        if e then
            ...
            for _,line in ipairs(e.legacyBox or {}) do line.Visible=false end
            for _,line in ipairs(e.skeleton or {}) do line.Visible=false end
        end
    end

    if not Config.ESP then
        -- Do a real teardown, not just Visible=false. Some compatibility
        -- primitives are siblings of the player container and otherwise can
        -- survive a later CreateESP()/respawn cycle as orphaned UI.
        local doomed={}
                ...
                if distWorld<=Config.ESPMaxDistance then
                    ...
                    if e then
                        ...
                        if onScreen and rootZ>0 then
                            ...
                            -- Box bounds come from the actual character model, not an
                            -- arbitrary RootPart-centered estimate.
                            if Config.ESPBoxes then
                                local minX,minY,maxX,maxY=GetCharacterScreenBounds(cam,char,view)
                                if minX then
                                    pcall(UpdateLegacyBox,e,minX,minY,maxX,maxY,color)
                                else
                                    for _,v in ipairs(e.legacyBox or {}) do v.Visible=false end
                                end
                            else
                                for _,v in ipairs(e.legacyBox or {}) do v.Visible=false end
                            end

                            local tracerPoint=cam:WorldToViewportPoint(head and head.Position or root.Position)
                            pcall(UpdateTracerLegacy,e.tracer,tracerPoint,color)
                            pcall(UpdateSkeleton,e,char,color,cam)
                        else
