...
--========================================================
-- ESP SYSTEM 3.0 — pooled, single-owner, teardown-safe
--========================================================
-- Design goals:
--   1) ONE root object owns every screen-space primitive for a player.
--   2) Turning a feature off hides it immediately; turning master ESP off
--      destroys the whole pool, so no orphaned boxes/arrows/tracers/skeletons
--      can survive a respawn or a second execution of the script.
--   3) Character changes are reconciled, not rebuilt every frame.
--   4) Screen-space ESP lives in an unscaled ScreenGui, so mobile UIScale
--      cannot distort WorldToViewportPoint coordinates.
--   5) Optional features are pooled and only updated when enabled.

local function DestroyNamedESPObjects(parent)
    if not parent then return end
    for _,obj in ipairs(parent:GetChildren()) do
        if obj.Name=="FPSESPOverlay" or obj:GetAttribute("FPSESPManaged") then
            pcall(function() obj:Destroy() end)
        end
    end
end

-- Clean remnants from older executions/versions once at startup.
DestroyNamedESPObjects(PlayerGui)
for _,obj in ipairs(PlayerGui:GetDescendants()) do
    if obj:GetAttribute("FPSESPManaged") or obj.Name=="ESPHighlight" or obj.Name=="FPSESPHighlight" then
        pcall(function() obj:Destroy() end)
    end
end
...
local function SetVisible(obj,state)
    if obj then obj.Visible=state==true end
end
...
local function MakeScreenLine(parent,zIndex,thickness)
    return New("Frame",{
        ...
        BorderSizePixel=0,
        Size=UDim2.fromOffset(thickness or 2,1),
        Visible=false,
        ZIndex=zIndex or 40,
        Active=false,
    },parent)
...
local function IsInsideViewport(point,view,margin)
    margin=margin or 0
    return point.X>=-margin and point.X<=view.X+margin
        and point.Y>=-margin and point.Y<=view.Y+margin
end

local function GetCharacterScreenBounds(cam,char,view)
    -- Character bounding box + all 8 corners. This keeps the box attached to
    -- the actual avatar rather than HumanoidRootPart-only estimates.
    ...
    for sx=-1,1,2 do
        for sy=-1,1,2 do
            for sz=-1,1,2 do
                local wp=(boundsCF*CFrame.new(sx*hx,sy*hy,sz*hz)).Position
    ...
    -- Keep coordinates bounded so giant partially-visible models never create
    -- absurdly huge GUI sizes.
...
local function PositionLine(line,a,b,thickness)
    ...
    line.Size=UDim2.fromOffset(math.max(thickness or 2,len),thickness or 2)
    line.Rotation=math.deg(math.atan2(d.Y,d.X))
    line.Visible=true
end

local function BuildCornerBox(parent)
    local box={segments={},shadow={}}
    for i=1,8 do
        box.segments[i]=New("Frame",{
            BackgroundColor3=C.Accent,
            BorderSizePixel=0,
            Visible=false,
            ZIndex=45,
...
local function UpdateCornerBox(e,minX,minY,maxX,maxY,color)
    local box=e.box
    if not box or not Config.ESPBoxes or not minX or not minY or not maxX or not maxY
        or maxX-minX<3 or maxY-minY<6 then
        if box then
            for i=1,#box.segments do
                box.segments[i].Visible=false
                box.shadow[i].Visible=false
            end
        end
        return
    end

    local w=math.max(4,maxX-minX)
    local h=math.max(8,maxY-minY)
    local corner=math.clamp(math.min(w*.28,h*.18),10,34)
    local thickness=math.clamp(math.min(w,h)*.012,2,3)
    ...
    for i,d in ipairs(defs) do
        local a,b=Vector2.new(d[1],d[2]),Vector2.new(d[3],d[4])
        local s=box.segments[i]
        local sh=box.shadow[i]
        PositionLine(s,a,b,thickness)
        s.BackgroundColor3=color
        s.ZIndex=45
        -- Shadow is a second pass offset by one pixel. It improves visibility
        -- without changing the actual box geometry.
        PositionLine(sh,a+Vector2.new(shadowOffset,shadowOffset),b+Vector2.new(shadowOffset,shadowOffset),thickness+1)
        sh.Visible=true
    end
end

local function BuildTracer(parent)
    return {
        shadow=MakeScreenLine(parent,38,4),
        line=MakeScreenLine(parent,39,2),
    }
end

local function UpdateTracer(e,toPoint,color,view)
    if not e.tracer or not Config.ESPTracers then
        if e.tracer then e.tracer.line.Visible=false; e.tracer.shadow.Visible=false end
        return
    end
    local from=Vector2.new(view.X*.5,view.Y)
    if not IsInsideViewport(toPoint,view,120) then
        e.tracer.line.Visible=false
        e.tracer.shadow.Visible=false
        return
    end
    local d=toPoint-from
    if d.Magnitude<4 then
        e.tracer.line.Visible=false
        e.tracer.shadow.Visible=false
        return
    end
    PositionLine(e.tracer.shadow,from+Vector2.new(1,1),toPoint+Vector2.new(1,1),4)
    PositionLine(e.tracer.line,from,toPoint,2)
    e.tracer.line.BackgroundColor3=color
end

local function BuildSkeleton(parent)
    local skeleton={}
    local max=#R15Bones
    for i=1,max do
        skeleton[i]=MakeScreenLine(parent,42,2)
    end
    return skeleton
end
...
local function UpdateSkeleton(e,char,color,cam,view)
    if not e.skeleton or not Config.ESPSkeleton then
        if e.skeleton then
            for _,line in ipairs(e.skeleton) do line.Visible=false end
        end
        return
    end
    ...
    for i=1,#e.skeleton do
        ...
        if pair then
            ...
            if a and b then
                local pa,za=Project(cam,a.Position)
                local pb,zb=Project(cam,b.Position)
                if za>0 and zb>0 and IsInsideViewport(pa,view,90) and IsInsideViewport(pb,view,90) then
                    line.BackgroundColor3=color
                    PositionLine(line,pa,pb,1.75)
                else
                    line.Visible=false
                end
            else
...
local function BuildESP(player,char)
    ...
    if existing then
        -- Destroy the complete previous generation before creating another.
    ...
    root:SetAttribute("FPSESPManaged",true)
    ...
    Corner(card,10)
    Outline(card,Color3.fromRGB(55,60,72))
    ...
    local user=New("TextLabel",{
        Size=UDim2.fromOffset(118,14),
        Position=UDim2.fromOffset(20,23),
        BackgroundTransparency=1,
        Text="@"..player.Name,
    ...
    local dist=New("TextLabel",{
        ...
        Position=UDim2.fromOffset(126,7),
        BackgroundTransparency=1,
        TextColor3=Color3.fromRGB(190,195,207),
        TextSize=9,
        Font=Enum.Font.GothamBold,
    ...
    local hpBack=New("Frame",{
        Size=UDim2.fromOffset(148,5),
        Position=UDim2.fromOffset(20,39),
        BackgroundColor3=Color3.fromRGB(33,37,45),
        BorderSizePixel=0,
        ZIndex=31,
    },card)
    Corner(hpBack,3)
    ...
    local hpText=New("TextLabel",{
        ...
        Position=UDim2.fromOffset(116,46),
        BackgroundTransparency=1,
        TextColor3=Color3.fromRGB(190,195,207),
        TextSize=8,
        Font=Enum.Font.Gotham,
    ...
    local teamDot=New("Frame",{
        AnchorPoint=Vector2.new(.5,.5),
        Size=UDim2.fromOffset(7,7),
        Position=UDim2.fromOffset(179,47),
        BackgroundColor3=GetESPColor(player),
        BorderSizePixel=0,
        ZIndex=33,
        Visible=true,
...
local function ClearAllESP()
    ...
    for _,plr in ipairs(doomed) do
        RemoveESP(plr)
    end
end

local function SetESPFeatureVisibility(e,visible)
    if not visible then
...
local function UpdateOffscreenArrow(e,screenPoint,color,view)
    if not Config.ESPOffscreen then
    ...
    -- For targets behind the camera, flip the projected vector so the arrow
    -- still points toward the correct world direction.
    local n=d.Unit
    local margin=34
    ...
    local tx=math.huge
    local ty=math.huge
    if math.abs(n.X)>1e-4 then tx=halfX/math.abs(n.X) end
    if math.abs(n.Y)>1e-4 then ty=halfY/math.abs(n.Y) end
...
UpdateESP=function()
    ...
    if not Config.ESP then
        -- Master OFF = complete teardown. No stale primitives, no hidden
        -- survivors, no waiting for the next render tick.
        ClearAllESP()
        return
    end
    ...
    -- Keep the pool synchronized without creating/destroying objects on every
    -- render pass unless a character actually changed.
        ...
        if char and hum and rootPart and hum.Health>0 then
            ...
            if distance<=Config.ESPMaxDistance then
                ...
                if e then
                    ...
                    local onScreen=inFront and IsInsideViewport(root2d,view,0)

                    e.character=char
                    e.highlight.Adornee=char
                    e.highlight.OutlineColor=color
                    e.highlight.FillColor=color
                    e.highlight.Enabled=Config.ESPHighlight and inFront
                    e.accent.BackgroundColor3=color
                    e.teamDot.BackgroundColor3=color
                    e.headDot.BackgroundColor3=color

                    if onScreen then
                        SetESPFeatureVisibility(e,true)

                        -- Nameplate is anchored above the player's head, with a
                        -- small safety offset so it never touches the avatar.
                        e.card.Visible=Config.ESPNames or Config.ESPHealth or Config.ESPDistance
                        e.card.Position=UDim2.fromOffset(head2d.X,head2d.Y-14)
                        ...
                        e.headDot.Visible=Config.ESPHeadDot and head and headZ>0
                        ...
                        if minX and Config.ESPBoxes then
                            UpdateCornerBox(e,minX,minY,maxX,maxY,color)
                        else
                            UpdateCornerBox(e,0,0,0,0,color)
                        end
                        ...
                        -- On-screen card/box/skeleton/tracer state is always
                        -- explicitly reset before considering the arrow.
                        e.root.Visible=true
                        ...
                        if inFront then
                            UpdateOffscreenArrow(e,root2d,color,view)
                        else
                            -- For behind-camera targets, use a point mirrored
                            -- through the viewport center to get stable arrow
                            -- orientation instead of a random corner.
                            local mirrored=Vector2.new(view.X-root2d.X,view.Y-root2d.Y)
                            UpdateOffscreenArrow(e,mirrored,color,view)
                        end
                    end
                end
            end
    ...
    -- Any pooled object not seen in this pass is fully hidden. We keep the
    -- object alive for reuse when the player comes back into range.
        ...
        if not seen[plr] and e then
            ...
            if e.box then
                for i=1,#e.box.segments do
                    e.box.segments[i].Visible=false
                    e.box.shadow[i].Visible=false
                end
            end
            if e.skeleton then
                for _,line in ipairs(e.skeleton) do line.Visible=false end
