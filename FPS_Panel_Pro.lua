...
--========================================================
-- ESP SYSTEM 4.0 — single-owner, deterministic, no-orphans
--========================================================
-- IMPORTANT: This block is isolated from the aimbot. It does not modify any
-- aim state, camera state, aim settings, or the aimbot render binding.
-- Every 2D primitive for a player is owned by that player's root Frame.
-- A master-off destroys the entire ESP tree and performs a final sweep.

local function DestroyNamedESPObjects(parent)
    if not parent then return end
    for _,obj in ipairs(parent:GetChildren()) do
        if obj:GetAttribute("FPSESPManaged") or obj.Name=="FPSESPOverlay" then
            pcall(function() obj:Destroy() end)
        end
    end
end

-- Kill remnants from previous generations before creating this generation.
DestroyNamedESPObjects(PlayerGui)
...
espOverlay:SetAttribute("FPSESPManaged",true)
...
local function AddESPPlayerCache(plr)
    if plr==LocalPlayer then return end
    for i=1,#espPlayerCache do
        if espPlayerCache[i]==plr then return end
    end
    espPlayerCache[#espPlayerCache+1]=plr
end

local function RemoveESPPlayerCache(plr)
...
Players.PlayerAdded:Connect(AddESPPlayerCache)
Players.PlayerRemoving:Connect(RemoveESPPlayerCache)
...
local function NewESPObject(class,props,parent)
    local obj=New(class,props,parent)
...
local function HidePlayerESP(e)
    ...
    if e.root then e.root.Visible=false end
    ...
    if e.tracer then
        HideLine(e.tracer.line)
        HideLine(e.tracer.shadow)
    end
    if e.box then
        for i=1,#e.box.segments do
            HideLine(e.box.segments[i])
            HideLine(e.box.shadow[i])
        end
    end
    if e.skeleton then
        for _,line in ipairs(e.skeleton) do HideLine(line) end
    end
end

local function MakeScreenLine(parent,zIndex,thickness)
    return NewESPObject("Frame",{
        AnchorPoint=Vector2.new(.5,.5),
        BackgroundColor3=C.Accent,
        BorderSizePixel=0,
        Size=UDim2.fromOffset(thickness or 2,thickness or 2),
        Visible=false,
        ZIndex=zIndex or 40,
        Active=false,
    },parent)
...
local function Project(cam,pos)
    local p,onScreen=cam:WorldToViewportPoint(pos)
    return Vector2.new(p.X,p.Y),p.Z,onScreen
end

local function Line2D(line,a,b,thickness,color)
...
local function GetCharacterScreenBounds(cam,char,view)
    local ok,boundsCF,boundsSize=pcall(function()
        return char:GetBoundingBox()
    end)
    if not ok or not boundsCF or not boundsSize then return nil end

    local hx,hy,hz=boundsSize.X*.5,boundsSize.Y*.5,boundsSize.Z*.5
    ...
    for sx=-1,1,2 do
        for sy=-1,1,2 do
            for sz=-1,1,2 do
                local worldPoint=(boundsCF*CFrame.new(sx*hx,sy*hy,sz*hz)).Position
                local p=cam:WorldToViewportPoint(worldPoint)
                if p.Z>0 then
                    anyFront=true
    ...
    if maxX-minX<3 or maxY-minY<6 then return nil end

    minX=math.clamp(minX,-120,view.X+120)
    maxX=math.clamp(maxX,-120,view.X+120)
    minY=math.clamp(minY,-120,view.Y+120)
    maxY=math.clamp(maxY,-120,view.Y+120)
    return minX,minY,maxX,maxY
end

local function BuildCornerBox(root)
    local box={segments={},shadow={}}
    for i=1,8 do
        box.segments[i]=NewESPObject("Frame",{
            BackgroundColor3=C.Accent,
            BorderSizePixel=0,
            Visible=false,
            ZIndex=50,
        ...
        box.shadow[i]=NewESPObject("Frame",{
...
local function UpdateCornerBox(e,minX,minY,maxX,maxY,color)
    local box=e and e.box
    if not box or not Config.ESPBoxes or not minX or not minY or not maxX or not maxY then
        if box then
            for i=1,#box.segments do
                HideLine(box.segments[i]); HideLine(box.shadow[i])
            end
        end
        return
    end
    ...
    local corner=math.clamp(math.min(width*.25,height*.18),9,34)
    local thickness=math.clamp(math.min(width,height)*.015,2,3)
    local defs={
    ...
    for i,d in ipairs(defs) do
        Line2D(box.segments[i],Vector2.new(d[1],d[2]),Vector2.new(d[3],d[4]),thickness,color)
        Line2D(box.shadow[i],Vector2.new(d[1]+1,d[2]+1),Vector2.new(d[3]+1,d[4]+1),thickness+1,Color3.new(0,0,0))
    end
end

local function BuildTracer(root)
    return {
        shadow=MakeScreenLine(root,44,4),
        line=MakeScreenLine(root,45,2),
    }
end

local function UpdateTracer(e,targetPoint,color,view)
    if not e.tracer or not Config.ESPTracers or not targetPoint then
        if e.tracer then HideLine(e.tracer.line); HideLine(e.tracer.shadow) end
        return
    end
    local from=Vector2.new(view.X*.5,view.Y-2)
    local to=targetPoint
    if to.X<-100 or to.X>view.X+100 or to.Y<-100 or to.Y>view.Y+100 then
        HideLine(e.tracer.line); HideLine(e.tracer.shadow)
        return
    end
    local d=to-from
    ...
    Line2D(e.tracer.shadow,from+Vector2.new(1,1),to+Vector2.new(1,1),4,Color3.new(0,0,0))
    Line2D(e.tracer.line,from,to,2,color)
end

local function BuildSkeleton(root)
    local skeleton={}
    local max=#R15Bones
    for i=1,max do skeleton[i]=MakeScreenLine(root,47,2) end
    return skeleton
end

local function UpdateSkeleton(e,char,color,cam,view)
    if not e.skeleton or not Config.ESPSkeleton then
        if e.skeleton then for _,line in ipairs(e.skeleton) do HideLine(line) end end
        return
    end
    local defs=GetSkeletonBones(char)
    for i=1,#e.skeleton do
        ...
        if not pair then
            ...
            if a and b then
                local pa,za,ona=Project(cam,a.Position)
                local pb,zb,onb=Project(cam,b.Position)
                if za>0 and zb>0 and (ona or onb)
                    and pa.X>-120 and pa.X<view.X+120 and pa.Y>-120 and pa.Y<view.Y+120
                    and pb.X>-120 and pb.X<view.X+120 and pb.Y>-120 and pb.Y<view.Y+120 then
                    Line2D(line,pa,pb,1.75,color)
                else
                    HideLine(line)
                end
            else
...
local function BuildESP(player,char)
    ...
    if existing and existing.character==char and existing.root and existing.root.Parent then
        return existing
    end
    ...
    local root=NewESPObject("Frame",{
        Name="Player_"..tostring(player.UserId),
        Size=UDim2.fromScale(1,1),
        Position=UDim2.fromScale(0,0),
        BackgroundTransparency=1,
        BorderSizePixel=0,
    ...
    local highlight=NewESPObject("Highlight",{
        Name="FPSESPHighlight",
        Adornee=char,
        DepthMode=Enum.HighlightDepthMode.AlwaysOnTop,
        FillTransparency=.94,
        OutlineTransparency=.04,
        FillColor=GetESPColor(player),
        OutlineColor=GetESPColor(player),
        Enabled=false,
    },workspace)

    local card=NewESPObject("Frame",{
    ...
    Corner(card,10); Outline(card,Color3.fromRGB(55,60,72))

    local accent=NewESPObject("Frame",{
        Size=UDim2.fromOffset(4,40),Position=UDim2.fromOffset(8,9),
        BackgroundColor3=GetESPColor(player),BorderSizePixel=0,ZIndex=31,
    },card)
    Corner(accent,2)

    local name=NewESPObject("TextLabel",{
        Size=UDim2.fromOffset(118,18),Position=UDim2.fromOffset(20,6),BackgroundTransparency=1,
        Text=player.DisplayName,TextColor3=Color3.new(1,1,1),TextSize=12,Font=Enum.Font.GothamBold,
        TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd,ZIndex=32,
    },card)
    local user=NewESPObject("TextLabel",{
        Size=UDim2.fromOffset(118,14),Position=UDim2.fromOffset(20,23),BackgroundTransparency=1,
        Text="@"..player.Name,TextColor3=Color3.fromRGB(145,150,165),TextSize=9,Font=Enum.Font.Gotham,
        TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd,ZIndex=32,
    },card)
    local dist=NewESPObject("TextLabel",{
        Size=UDim2.fromOffset(54,16),Position=UDim2.fromOffset(126,7),BackgroundTransparency=1,
        TextColor3=Color3.fromRGB(190,195,207),TextSize=9,Font=Enum.Font.GothamBold,
        TextXAlignment=Enum.TextXAlignment.Right,ZIndex=32,
    },card)
    local hpBack=NewESPObject("Frame",{
        Size=UDim2.fromOffset(148,5),Position=UDim2.fromOffset(20,39),BackgroundColor3=Color3.fromRGB(33,37,45),
        BorderSizePixel=0,ZIndex=31,
    },card); Corner(hpBack,3)
    local hpFill=NewESPObject("Frame",{
        Size=UDim2.fromScale(1,1),BackgroundColor3=C.Good,BorderSizePixel=0,ZIndex=32,
    },hpBack); Corner(hpFill,3)
    local hpText=NewESPObject("TextLabel",{
        Size=UDim2.fromOffset(52,13),Position=UDim2.fromOffset(116,46),BackgroundTransparency=1,
        TextColor3=Color3.fromRGB(190,195,207),TextSize=8,Font=Enum.Font.Gotham,TextXAlignment=Enum.TextXAlignment.Right,ZIndex=32,
    },card)
    local teamDot=NewESPObject("Frame",{
        AnchorPoint=Vector2.new(.5,.5),Size=UDim2.fromOffset(7,7),Position=UDim2.fromOffset(179,47),
        BackgroundColor3=GetESPColor(player),BorderSizePixel=0,ZIndex=33,Visible=true,
    },card); Corner(teamDot,4)

    local headDot=NewESPObject("Frame",{
        AnchorPoint=Vector2.new(.5,.5),Size=UDim2.fromOffset(7,7),BackgroundColor3=GetESPColor(player),
        BorderSizePixel=0,ZIndex=53,Visible=false,
    },root); Corner(headDot,4)

    local arrow=NewESPObject("TextLabel",{
        AnchorPoint=Vector2.new(.5,.5),Size=UDim2.fromOffset(34,34),BackgroundTransparency=1,
        Text="▲",TextColor3=GetESPColor(player),TextStrokeColor3=Color3.new(0,0,0),TextStrokeTransparency=.15,
        TextSize=24,Font=Enum.Font.GothamBlack,ZIndex=55,Visible=false,
    },root)

    local box=BuildCornerBox(root)
    local tracer=BuildTracer(root)
    local skeleton=BuildSkeleton(root)

    local destroyed=false
    local e={
        player=player,character=char,root=root,highlight=highlight,card=card,
        accent=accent,name=name,user=user,dist=dist,hpBack=hpBack,hpFill=hpFill,hpText=hpText,
        teamDot=teamDot,headDot=headDot,arrow=arrow,box=box,tracer=tracer,skeleton=skeleton,
    }
    e.destroy=function()
...
ClearAllESP=function()
    local doomed={}
    for plr in pairs(espObjects) do doomed[#doomed+1]=plr end
    for _,plr in ipairs(doomed) do RemoveESP(plr) end

    -- Last-resort sweep: if a previous generation escaped the table, remove it.
    for _,obj in ipairs(ESPGui:GetDescendants()) do
        if obj:GetAttribute("FPSESPManaged") and obj~=ESPGui then
            pcall(function() obj:Destroy() end)
        end
    end
    for _,obj in ipairs(workspace:GetChildren()) do
...
local function HideFeaturePrimitives(e)
    if not e then return end
    if e.card then e.card.Visible=false end
    if e.headDot then e.headDot.Visible=false end
    if e.arrow then e.arrow.Visible=false end
    if e.tracer then HideLine(e.tracer.line); HideLine(e.tracer.shadow) end
    if e.box then
        for i=1,#e.box.segments do HideLine(e.box.segments[i]); HideLine(e.box.shadow[i]) end
    end
    if e.skeleton then for _,line in ipairs(e.skeleton) do HideLine(line) end end
end

local function UpdateOffscreenArrow(e,screenPoint,color,view,behind)
    ...
    local n=d.Unit
    local halfW=view.X*.5-42
    local halfH=view.Y*.5-42
    local tx=math.abs(n.X)>1e-4 and halfW/math.abs(n.X) or math.huge
    local ty=math.abs(n.Y)>1e-4 and halfH/math.abs(n.Y) or math.huge
...
UpdateESP=function()
    ...
    for _,plr in ipairs(espPlayerCache) do
        local char=plr.Character
        local hum=char and char:FindFirstChildOfClass("Humanoid")
        local rootPart=char and char:FindFirstChild("HumanoidRootPart")
        local head=char and char:FindFirstChild("Head")

        if char and hum and rootPart and hum.Health>0 then
            local distance=(rootPart.Position-localPosition).Magnitude
            if distance<=Config.ESPMaxDistance then
                local e=espObjects[plr]
                if not e or e.character~=char or not e.root or not e.root.Parent then
                    e=BuildESP(plr,char)
                end

                if e then
                    seen[plr]=true
                    -- Start every player update from a completely clean primitive state.
                    HidePlayerESP(e)
                    ...
                    local root2d,rootZ,rootOn=Project(cam,rootPart.Position)
                    local head2d,headZ,headOn=head and Project(cam,head.Position) or root2d,rootZ,rootOn
                    local inFront=rootZ>0
                    local onScreen=inFront and rootOn and root2d.X>=0 and root2d.X<=view.X and root2d.Y>=0 and root2d.Y<=view.Y
                    ...
                    if onScreen then
                        ...
                        e.card.Position=UDim2.fromOffset(head2d.X,head2d.Y-16)
                        ...
                        e.headDot.Visible=Config.ESPHeadDot and headOn and headZ>0
                        if e.headDot.Visible then e.headDot.Position=UDim2.fromOffset(head2d.X,head2d.Y) end

                        local minX,minY,maxX,maxY=GetCharacterScreenBounds(cam,char,view)
                        UpdateCornerBox(e,minX,minY,maxX,maxY,color)
                        UpdateTracer(e,root2d,color,view)
                        UpdateSkeleton(e,char,color,cam,view)
                    else
                        -- Only the arrow survives off-screen. Everything else was hidden above.
                        e.root.Visible=true
                        local arrowPoint=head and head2d or root2d
                        UpdateOffscreenArrow(e,arrowPoint,color,view,not inFront)
                    end

                    if not e.card.Visible
                        and not e.headDot.Visible
                        and not e.arrow.Visible
                        and not Config.ESPBoxes
                        and not Config.ESPTracers
                        and not Config.ESPSkeleton then
